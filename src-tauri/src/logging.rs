...
fn get_log_directory<R: Runtime>(app: &AppHandle<R>) -> Result<PathBuf, Box<dyn std::error::Error>> {
...
#[tauri::command]
pub async fn read_log_file<R: Runtime>(app: AppHandle<R>, file_name: String) -> Result<String, String> {
    let log_dir = match get_log_directory(&app) {
        Ok(dir) => dir,
        Err(e) => return Err(format!("Failed to get log directory: {}", e)),
    };
...
#[tauri::command]
pub async fn clear_log_file<R: Runtime>(app: AppHandle<R>, file_name: String) -> Result<(), String> {
    let log_dir = match get_log_directory(&app) {
        Ok(dir) => dir,
        Err(e) => return Err(format!("Failed to get log directory: {}", e)),
    };
...
#[tauri::command]
pub async fn cleanup_old_logs<R: Runtime>(app: AppHandle<R>, days_to_keep: u32) -> Result<u32, String> {
    let log_dir = match get_log_directory(&app) {
        Ok(dir) => dir,
        Err(e) => return Err(format!("Failed to get log directory: {}", e)),
    };
