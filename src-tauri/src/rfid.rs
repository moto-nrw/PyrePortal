...
// Global scanner guard used by BOTH continuous background scanning and one-shot scans.
// Serializes scanner access so a one-shot scan never overlaps the background loop.
